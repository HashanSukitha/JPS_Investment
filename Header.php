...
<script>
function autoSettlement(loanId)
{
j(".preloader").css("display", "block");
	sltmntAmt = j("#autoSltAmt").val();
	
	j.ajax(
				{
				type: 'post',
				url: 'phpFunctions/AutoSettlement_Update.php',
				data: {
						LoanId: loanId,
						SltmntAmt:sltmntAmt
				...
				success: function(result)
						{
						  ...
						  {
						    alert("Settled Successfully Remaining Amount = "+result);
						  }
							 j( "#container" ).load( "phpFunctions/ViewContractDet.php?Contr_No="+loanId );
							 j('.preloader').fadeOut('slow',function(){ j(".preloader").css("display", "none");});
							 //j('#temp').append(result);
							 //alert(result);
							 //j( "#UpdateDepositBox").slideUp( "slow", function() {});
